...
public class Protocol
{
    ...
    public Protocol(int id)
    {
        ...
        X509Certificate2 certificate = X509Certificate2.CreateFromPemFile("certificate.crt", "private.key");
        ...
        {
            _listener.Start();
            Console.WriteLine($"Patient {_id} - Listener started on port {_port}");
        }
        catch (HttpListenerException ex)
        {
            Console.WriteLine($"Failed to start listener on port {_port}: {ex.Message}");
        }
...
{
    HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, $"https://localhost:{3000 + targetId}/")
    {
...
private async Task Connect(int port)
{
    ...
    { 
        try
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, $"https://localhost:{port}/");
